...
def nova_tarefa(request, id_vaga):
    ...
    try:
        ...
        messages.add_message(request, constants.ERROR, 'Tarefa não criada! Erro.')
        return redirect(f'/vagas/vaga/{id_vaga}')
...
def realizar_tarefa(request, id):
    ...
    if not tarefa_list.exists():
        messages.add_message(request, constants.ERROR, 'Realizar uma tarefa valida!')
        return redirect('/home/empresas/')

    tarefa = tarefa_list.first()
    tarefa.realizada = True
    tarefa.save()
    messages.add_message(request, constants.SUCCESS, 'Tarefa realizda com sucesso!')
...
def envia_email(request, id_vaga):
    ...
    html_content = render_to_string('emails/template_email.html',
                                    {'corpo': corpo}
                                    )
